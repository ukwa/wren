...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field 'status_s', label: 'Crawl Status'
    config.add_facet_field 'crawl_stream_s', label: 'Crawl Stream'
    ...
    config.add_facet_field 'last_updated_tdt_facet_field', label: 'Updated Date', :query => {
        :days_1 => { label: 'within 1 Day', fq: "last_updated_tdt:[#{(Time.zone.now - 1.days).utc.iso8601} TO *]" },
        :days_2 => { label: 'within 2 Days', fq: "last_updated_tdt:[#{(Time.zone.now - 2.days).utc.iso8601} TO *]" },
        :days_5 => { label: 'within 5 Days', fq: "last_updated_tdt:[#{(Time.zone.now - 5.days).utc.iso8601} TO *]" }
    }
